...
module.exports = (sheetID, documentID, columns) => {
  ...
  return new Promise((resolve, reject) => {
    if (fs.existsSync(cachePath) && process.env.ELEVENTY_ENV == "development") {
      ...
      console.log(
        `Fetching data from https://sheets.googleapis.com/v4/spreadsheets/${documentID}/values/${sheetID}?alt=json&key=AIzaSyDKG8NODyTNED_cjD0aMh0M2NFfH6Wn6FY`
      );

      axios
        .get(
          `https://sheets.googleapis.com/v4/spreadsheets/${documentID}/values/${sheetID}?alt=json&key=AIzaSyDKG8NODyTNED_cjD0aMh0M2NFfH6Wn6FY`
        )
        .then((response) => {
          ...
          response.data.values.forEach((row, index) => {
            ...
            columns.forEach((column) => {
              outputRow[column.toLowerCase()] = row[headers.indexOf(column)];
            });
            data.push(outputRow);
          });
